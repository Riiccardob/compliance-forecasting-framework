...
\textit{src/phase1/}  & \textit{feature\_selector.py}   & Fase~I         & Selezione di $M_{\Phi_i}$ dalla topologia \\
                      & \textit{stat\_forecaster.py}    & Fase~I         & Previsione locale delle metriche \\
...
compatibile con il \textit{framework} senza modificare una riga di codice al di fuori del
pacchetto \textit{etl/}. Tale componente è l'unico modulo \textit{dataset-specific}
...
un \textit{overhead} computazionale crescente con il numero di proprietà monitorate. Produce
...
La chiamata finale \textit{copy.deepcopy(self.\_graph)} implementa il Pattern Prototype: il
...
finestre presenti simultaneamente in tutti e tre. Pre-indicizza i DataFrame per \textit{timestamp}
con \textit{groupby} prima del ciclo principale, riducendo il costo di \textit{lookup} da $O(n)$
...
direttamente consumabili dal FeatureSelector senza ulteriori trasformazioni. Qualora
...
dell'identificatore. Questo comportamento è coerente con quello di \textit{build()}, che
logga attivamente gli identificatori non riconosciuti durante la fase di costruzione:
...
for source_node, out_edges in node_out_edges.items():
    vals = [
        snapshot["edges"][eid].get(weight_metric, 0.0)
        for eid in out_edges
        if eid in snapshot["edges"]
    ]
    total = sum(v for v in vals if isinstance(v, (int, float))
                and not math.isnan(v) and v >= 0)
    if total <= 0 or any(
        not isinstance(v, (int, float)) or math.isnan(v) or v < 0
        for v in vals
    ):
        # Fallback uniforme: 1/n per tutti gli archi uscenti
        n = len(out_edges)
        logger.warning(
            "Throughput non valido per nodo '%s' (ts=%d): "
        ...
        for eid, v in zip(out_edges, vals):
            weights[eid] = max(0.0, v) / total
\end{lstlisting}
...
$W^{\text{gold}}[\text{e6}] = (0.0 + 0.6) / 2 = 0.3$, non il solo valore del secondo snapshot.
...
La Fase~I della \textit{pipeline} trasforma le serie temporali in $M_{\Phi_i}$ in previsioni
...
Il StatForecaster implementa il \textit{Pattern Strategy} per l'assegnazione dei modelli
...
sistema non tenti di addestrare un modello con memoria lagghata su serie troppo corte, producendo
...
\subsection{Le Tre Categorie di Coppie Candidate}
...
destinazione: la metrica di stato interno di un componente come causa e la latenza di un arco
...
Una quarta categoria, \texttt{inter2}, identifica le coppie
$(\texttt{node:}v\texttt{:metric},\ \texttt{edge:}e_{\text{int}}\texttt{:metric})$
con $v \in \text{Shared}(H_{\Phi_i}, H_{\Phi_j})$: la seconda
freccia della catena \textit{cross-property}
$\theta_{e_j} \to \texttt{node:}v \to \texttt{edge:}e_{\text{int}}$.
Come le coppie \texttt{inter}, bypassa il filtro di Pearson.
...
catena sono catturate e loggate come avvisi senza interrompere la \textit{pipeline}: il modulo
...
singola metrica è fuori soglia e IF conferma l'anomalia aggregata; richiederebbbe un'architettura
...
Il metodo \texttt{reset\_cusum()} rimane necessario tra esperimenti distinti per azzerare
eventuali residui accumulati.

Il Livello~4, il Validatore strutturale, è attivato solo quando sia \textit{if\_signal} sia
\textit{cusum\_signal} sono veri. Verifica due condizioni congiunte: la distanza strutturale
attuale supera \textit{distance\_threshold} (default 0,15):
per le topologie lineari questo corrisponde al
PAS-gap $|\text{PA}(t) - \text{PAS}_{\text{gold}}| > \delta$
come descritto in~\cite[§3.2.3]{methodology};
per le topologie parallele alla norma di Frobenius
$\|W(t) - W^{\text{gold}}\|_F > \delta$.
...
La suite copre complessivamente oltre 293 casi distribuiti in dieci file. I tipi di test
