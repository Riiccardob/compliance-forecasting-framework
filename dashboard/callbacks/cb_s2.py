...
_COUNTS = {
    "H_crit":  {"node": 20, "edge": 12, "interf": 0,  "total": 32},
    "H_cache": {"node": 16, "edge": 9,  "interf": 1,  "total": 26},
}
...
def update_cs(cs):
    c = _COUNTS.get(cs, {"node": 0, "edge": 0, "interf": 0, "total": 0})
    cards = [
        _count_card("M_direct", str(c["node"] + c["edge"]), "#c4a35a"),
        _count_card("M_interf", str(c["interf"]),           "#8957e5"),
        _count_card("Totale",   str(c["total"]),            "#e2ddd5"),
    ]
    ...
    if results and cs in results.get("compliance_sets", {}):
        ...
        opts    = [{"label": k, "value": k} for k in sorted(features.keys())]
        default = opts[0]["value"] if opts else None
    else:
        opts, default = [], None

    return cards, opts, default
