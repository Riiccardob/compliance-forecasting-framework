...
def show_edge_detail(edge_data):
    if not edge_data:
        return html.Div("Clicca un arco causale.",
        ...
        for e in cs_data.get("causal_graph", {}).get("edges", []):
            ...
            if eid == edge_id:
                rows = [
                    ...
                    ("Tipo",      e.get("type", "")),
                    ("Metodo",    e.get("method", "")),
                    ("Intensita", f"{e.get('intensity', 0):.4f}"),
                    ("Lag",       str(e.get("lag", "N/A"))),
                ]
                title = (
                    f"{edge_data.get('source', '').split(':')[-1]}"
                    f" -> {edge_data.get('target', '').split(':')[-1]}"
                )
                return _detail_panel(title, rows)
