...
def _worker_pipeline(mode: str) -> None:
    _S["pipe_running"] = True
    _S["pipe_error"]   = ""
    _S["pipe_pct"]     = 5
    _S["pipe_msg"]     = "Avvio pipeline..."
    ...
    try:
        ...
        from dashboard.core.data_manager import DataManager as _DM

        dm = _DM()
        if not dm.is_data_loaded():
            raise RuntimeError("ATG non caricato. Esegui prima Carica dati.")

        all_snaps = dm.get_snapshots()
        _ts_idx   = {s["timestamp"]: i
        ...
        if mode == "full":
            ...
            if anomalous:
                ...
                indices = [_ts_idx[ts]] if ts in _ts_idx else [0]
            else:
                indices = [0]

        results = run_pipeline("DEMO" if mode == "sample" else "FULL",
                               indices, _progress)

        if "error" in results:
            raise RuntimeError(results["error"])

        n_snap = results.get("n_snapshots", 0)
        cs_summary = []
        for cs, d in results.get("compliance_sets", {}).items():
            na = len(d.get("alerts", []))
            cs_summary.append(f"{cs}: {na} alert")

        _S["pipe_pct"]  = 100
        _S["pipe_msg"]  = (
            f"Pipeline completata. {n_snap} snapshot. "
            + " | ".join(cs_summary)
        )
        _S["pipe_done"] = True

    except Exception as exc:
        _S["pipe_error"] = str(exc)
        _S["pipe_pct"]   = 0
        _S["pipe_msg"]   = f"Errore: {exc}"
    finally:
        _S["pipe_running"] = False
...
# ── Callback: click Esegui pipeline ──────────────────────────────────────

@callback(
    Output("s0-btn-run", "children"),
    Input("s0-btn-run", "n_clicks"),
    State("s0-mode",    "value"),
    prevent_initial_call=True,
)
def start_pipeline(n_clicks, mode):
    if not n_clicks:
        return "Esegui pipeline"
    if _S["pipe_running"]:
...
def poll(n):
    ...
    btn_run_active    = {**_BTN_BASE, "backgroundColor": "var(--crit)",
                         "color": "#fff", "marginTop": "16px", "opacity": "1"}
    btn_run_disabled  = {**_BTN_BASE, "backgroundColor": "var(--border)",
                         "color": "var(--muted)", "marginTop": "16px",
                         "opacity": "0.4", "cursor": "not-allowed"}
    btn_run_running   = {**_BTN_BASE, "backgroundColor": "var(--border)",
                         "color": "var(--muted)", "marginTop": "16px",
                         "opacity": "0.6", "cursor": "not-allowed"}
    ...
    # --- Pipeline section ---
    pipe_pct   = _S["pipe_pct"]
    pipe_msg   = _S["pipe_msg"]
    pipe_error = _S["pipe_error"]

    atg_ready = _S["atg_done"] or DataManager().is_data_loaded()

    if pipe_error:
        run_log         = pipe_error
        btn_run_dis     = not atg_ready
        btn_run_style   = btn_run_active if atg_ready else btn_run_disabled
    elif _S["pipe_done"]:
        run_log         = pipe_msg
        btn_run_dis     = False
        btn_run_style   = btn_run_active
    elif _S["pipe_running"]:
        run_log         = pipe_msg
        btn_run_dis     = True
        btn_run_style   = btn_run_running
    else:
        run_log         = ""
        btn_run_dis     = not atg_ready
        btn_run_style   = btn_run_active if atg_ready else btn_run_disabled
    ...
    return (
        ...
        pipe_pct, pipe_msg,
        run_log,
        btn_run_dis, btn_run_style,
        snap_table,
    )


# ── Callback: toggle warning full dataset ────────────────────────────────

@callback(
    Output("s0-full-warning", "style"),
    Input("s0-mode", "value"),
)
def toggle_warning(mode):
    if mode == "full":
        return {"fontSize": "11px", "color": "var(--danger)",
